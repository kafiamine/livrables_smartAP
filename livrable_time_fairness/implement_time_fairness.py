...
Connected_nodes=upload_table("liste_connected.txt")  # recupere la liste des clients connectes d'un fichier sous forme: @IP demande debit 
#chaque ligne contient un client. la demande et le debit sont en Megabits per second (mbit), exemple: 
#3.10.10.10 100 144
#3.10.10.11 50 50
 
Disconnected_nodes=upload_table("liste_disconected.txt") # recupere la liste des clients deconnectes. On a besoin que de l'adresse IP
...
#c_code="tc qdisc add dev wlp2s0 root handle 1: htb \n" +"tc class add dev wlp2s0 parent 1: classid 1:1 htb rate 150mbit ceil 150mbit \n\n"      #**********ces deux commandes doit etre executes qu'une seule fois au debut, puisque concerne le root seulement
...
for node in Disconnected_nodes: #commence par gerer les clients deconnectes, en suprimant: classe, queue et filtre correspandants 
	...
	c_code="tc qdisc delete dev wlp2s0 parent 1:%d handle %d: pfifo \n"%(id_classe,id_classe+100)
	f1.write(c_code)

	c_code="tc filter del dev wlp2s0 parent 1: proto ip prio 1 handle 800::%d u32 \n"%(id_filter)
	f1.write(c_code)

	c_code="tc class del dev wlp2s0 parent 1: classid 1:%d \n\n"% id_classe
	f1.write(c_code)
...
for node in Connected_nodes:   #gerer les nouveaux clients connectes en leur creants: classe, queue, filtre. Modifier aussi les capacites de ceux existants, si besoin   
	...
	if (existing_node==True):
		...
		c_code="tc class change dev wlp2s0 parent 1:1 classid 1:%d htb rate %.2fmbit ceil %dmbit \n\n"%(id_classe,min(float(int(node[2])/float(nb_nodes)),int(node[1])),int(node[2])) 
		f1.write(c_code)

	else:
		c_code="tc class add dev wlp2s0 parent 1:1 classid 1:%d htb rate %.2fmbit ceil %dmbit \n"%(id_classe,min(float(int(node[2])/float(nb_nodes)),int(node[1])),int(node[2])) 
		f1.write(c_code)
		
		c_code="tc filter add dev wlp2s0 parent 1:0 prio 1 protocol ip u32 match ip dst %s classid 1:%d \n"%(node[0],id_classe)
		f1.write(c_code)
				
		c_code="tc qdisc add dev wlp2s0 parent 1:%d handle %d: pfifo limit %d \n\n"%(id_classe,id_classe+100,fifo_limit)	
		f1.write(c_code)
